...
async function registerToolFromOperation(operationFileRelativePath) {
    ...
    for (const key of requiredKeys) {
        if (!operation[key]) {
            throw new Error(`Parameter '${key}' in '${operationFileRelativePath}' is not well-defined`);
        }
    }
    ...
    if (!customBaseUrl.startsWith("http://") &&
        ...
        throw new Error(`Base URL must start with 'http://' or 'https://', received '${customBaseUrl}'`);
    }
